...
const ProductInspector3D = forwardRef(function ProductInspector3D({
  ...
  // inspection.id changes for every newly opened product, capturing a fresh anchor.
  ...
  const buttonAnimations = inspection.exposeAnimations === false ? [] : animations.slice(0, 5)
  ...
  return (
    ...
    >
      ...
      {buttonAnimations.map((name, index) => (
        <WorldButton
          key={name}
          label={name}
          position={[
            buttonAnimations.length > 1 ? (index - (buttonAnimations.length - 1) / 2) * 0.78 : 0,
            -0.28,
            0.82,
          ]}
          width={0.72}
          onActivate={() => playAnimation(name)}
        />
      ))}
